...
export class FileEndpointsClient {
    ...
    public async getRootFilesFilterDataAsync() {
        return await this.client.getAsync<StorageFileFilterDataModel>("api/file/root/files/filter");
    }
    ...
    public async getFilesByFilterAsync(filterOptions: StorageFileFilterModel) {
        ...
        return await this.client.getAsync<Array<StorageFile>>(`api/file/root/files/by-filter?${queryString}`);
    }
}